...
export const useResumeImportExport = () => {
    ...
    const exportResumes = async (resumeIds: string[]) => {
        ...
        const filename = resumes.length === 1
            ? `resume-${resumes[0].name.toLowerCase().replace(/\s+/g, '-')}-${timestamp}`
            : `resumes-export-${timestamp}`;

        // Use native share on mobile platforms
        if (isNativePlatform()) {
            const { shareJSON } = useNativeShare();
            await shareJSON(jsonData, filename);
            return;
        }
